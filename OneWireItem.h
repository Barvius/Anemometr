...
public:
    ...
    // takes ~(5.1-7.0)Вµs/byte (Atmega328P@16MHz) depends from address_size (see debug-crc-comparison.ino)
    ...
    // takes ~6Вµs/byte (Atmega328P@16MHz) (see debug-crc-comparison.ino)
    // important: the final crc is expected to be inverted (crc=~crc) !!!
